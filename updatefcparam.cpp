...
int main(int argc, char *argv[]) {
    ...
    if (valueType == ParamValueType::FLAOT) {
        ...
        if (param_value.first == Param::Result::Success && param_value.second == paramValue) {
            PLOGI.printf("参数已修改成功,%s:%f-->%f", paramKey.c_str(), oldValue, param_value.second);
            return 0;
        }
    ...
    int32_t newValue = static_cast<int32_t>(paramValue);
    // 发送参数修改请求
    Param::Result param_set_result = param.set_param_int(paramKey, newValue);
    if (param_set_result != Param::Result::Success) {
        PLOGE.printf("设置参数命令下发失败:%d", param_set_result);
        return 1;
    }
    ...
    // 读取参数值以确认修改
    param_value = param.get_param_int(paramKey);
    if (param_value.first == Param::Result::Success && param_value.second == newValue) {
        PLOGI.printf("参数已修改成功,%s:%d-->%d", paramKey.c_str(), oldValue, param_value.second);
        return 0;
    }
    PLOGI.printf("确认参数修改失败,当前值是:%d", param_value.second);
    return -1;
}
